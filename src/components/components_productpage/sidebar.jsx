...
function ComponentOption({ name, status, clickMethod }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        onClick={() => clickMethod(name)}
        className={
          "cursor-pointer  rounded w-[22px] h-[22px] flex justify-center items-center" +
          (status ? " bg-[#301C51]" : " border border-[#BBBBBB] bg-[#EAEAEA]")
        }
      >
        {status ? (
          <svg
          ...
          >
            <path
              d="M3.19691 8L0 4.80309L1.45689 3.3462L3.19691 5.09138L8.28314 0L9.74002 1.45689L3.19691 8Z"
              fill="#FBE146"
            />
          </svg>
        ) : (
      ...
      <p
        className={
          status
            ? "font-nunito text-darktext font-bold"
            : "font-nunito text-darktext"
        }
      >
        {t(name)}
      </p>
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        onClick={() => clickMethod(name)}
        className={
          "cursor-pointer rounded-full w-[22px] h-[22px] flex justify-center items-center" +
          (status ? " bg-[#301C51]" : " border border-[#BBBBBB] bg-[#EAEAEA]")
        }
      >
        {status ? (
      ...
      <div className="flex flex-col gap-8 px-5 border-r-[1px] border-[#CCC2BA] my-3 min-h-[75vh]">
        <div>
          <div className="flex justify-between items-center">
            <p className="font-quicksand font-semibold text-[20px]">
              {t("Category")}
            </p>
            {categoryBoleean ? (
              <svg
                onClick={() => setCategoryBoleean(false)}
              ...
              <svg
                onClick={() => setCategoryBoleean(true)}
                className="mt-1 cursor-pointer"
                xmlns="http://www.w3.org/2000/svg"
                width="14"
                height="8"
          ...
          {categoryBoleean ? (
            <>
              <div>
                {categories.filter((el) => el.status).length ? (
                  <div className="flex flex-wrap gap-2 pt-2 pb-4">
                    <p className="w-full font-quicksand font-semibold text-gray text-[16px]">
                      Selected Category
                    </p>
                    {categories
                      .filter((el) => el.status)
                      .map((el) => {
                        return (
                          <div
                            key={el.name}
                            className="cursor-pointer bg-[#EAEAEA] py-1 px-2.5 border border-[#BBBBBB] rounded-2xl flex items-center gap-2 justify-center"
                          >
                            <p className="font-nunito text-[16px] text-gray">
                              {t(el.name)}
                            </p>
                            <svg
                              onClick={() => clickStatusCategory(el.name)}
                              xmlns="http://www.w3.org/2000/svg"
                              width="11"
                              height="11"
                              viewBox="0 0 11 11"
                            ...
                            >
                              <path
                                d="M10.5 10.5L0.5 0.5M10.5 0.5L0.5 10.5"
                                stroke="#707070"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                              />
                    ...
                    <p
                      onClick={() => clickStatusCategory("All")}
                      className="cursor-pointer w-full font-quicksand text-[#ED4756] text-[14px] underline font-semibold"
                    >
                      {t("Delete all filter")}
                    </p>
                  </div>
              ...
              <div className="flex flex-col gap-3">
                <ComponentOption
                  clickMethod={clickStatusCategory}
                  name={"All"}
                  status={!categories.find(({ status }) => status === true)}
                />
                {categories.map((el, i) => (
                  <ComponentOption
                    clickMethod={clickStatusCategory}
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            ...
            {sortByBoleean ? (
              <svg
                onClick={() => setSortByBoleean(false)}
                className="mt-1 cursor-pointer"
                xmlns="http://www.w3.org/2000/svg"
                width="14"
                height="8"
