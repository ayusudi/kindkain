  ...
  return (
    <section className="bg-cream">
      ...
      <div className="max-w-screen-2xl m-auto px-5 hidden md:flex">
        <p className="px-5 py-5 text-[15px] font-quicksand">
          {t("Home")} /{" "}
          <span className="text-darkpurple">{t("Browse Products")} </span>
        </p>
      </div>
      <div className="max-w-screen-2xl m-auto flex flex-col md:flex-row md:px-5 pb-2 md:pb-20">
        ...
        <div className="flex flex-col flex-grow">
          <div className="relative flex md:flex-col justify-between pt-4 md:pt-0 pl-4 pr-5 xl:pl-10 2xl:pl-14 md:w-3/6 md:w-full text-[22px] md:text-[40px] font-quicksand font-bold text-darkpurple mb-3">
            <p>
              Browse <br className="flex md:hidden" />
              Our Products
            </p>
            ...
            {dataShow
              ...
              .map((el, i) => (
                <Link
                  ...
                  key={i}
                  className="cursor-pointer mb-3 flex flex-col items-center "
                >
                  <div className="w-[168px] md:w-[300px] flex flex-col gap-2 md:gap-3">
                    <div
                      className="relative w-[168px] h-[168px] md:w-[300px] md:h-[300px] animated-background bg-gradient-to-r from-zinc-200 via-zinc-200 to-zinc-400 rounded-xl"
                    ...
                    <p className="pl-1 font-nunito font-bold text-[16px] md:text-[21px] h-[70px] md:h-[64px] line-clamp-2 text-[#282525]">
                      {el.title}
                    </p>
                    <p className="pl-1 font-quicksand text-[16px] text-[#000000]">
                      {el.price}
                    </p>
                  </div>
                </Link>
              ))}
            {index < dataShow.length ? (
              <div
                onClick={() => setIndex(index + 6)}
                className="cursor-pointer hover:bg-darkpurple hover:text-white h-12 col-span-2 md:col-span-3 pl-6 text-[24px] text-center border border-1 rounded-full py-1.5 font-quicksand text-gray mb-6 mt-10"
              >
                <p>Load More</p>
              </div>
            ) : (
