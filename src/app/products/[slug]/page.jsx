      ...
      <div className="max-w-screen-2xl m-auto">
        <div className="cursor-pointer w-5/6 md:w-full px-5 hidden md:flex">
          <p className="px-5 pt-5 pb-3 text-[15px] font-quicksand">
            {t("Home")} / {t("Browse Product")} /{" "}
            <span
              className="cursor-pointer"
              onClick={() => movePage(product.category)}
            >
              {product.category}
            </span>{" "}
            /{" "}
          ...
          <div className="md:flex-1 md:pl-8 md:pr-8 flex flex-col gap-3 md:gap-6 items-start">
            <div>
              <h2
                ...
                className="md:mt-2.5 text-3xl text-center md:text-left md:text-5xl capitalize w-full md:w-5/6 font-quicksand font-bold text-darkpurple"
              >
                {product.name}
              </h2>
              <p className="text-xl md:text-[32px] text-center md:text-left font-nunito my-2 pt-1.5">
                {product.price}
              </p>
            </div>
            <p
              style={{ lineHeight: "150%", whiteSpace: "break-spaces" }}
              className="text-justify text-base md:text-[20px] text-gray font-nunito"
            >
              {product.description}
            </p>
            <Variants
            ...
            <button
              onClick={openNow}
              className="mt-2 md:mt=0 w-4/6 font-quicksand bg-darkpurple text-yellow text-md md:text-lg rounded-full px-4 py-2.5 font-semibold"
            >
              I Want This Product!
            </button>
            <Modal
